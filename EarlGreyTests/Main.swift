...
class MainEarlGreyTests: XCTestCase {
  ...
  func tryExpandRootMenu(isErrorFatal: Bool = false) {
    let expandItemMatcher = grey_allOfMatchers([
      grey_accessibilityID("Expand Root Menu"),
      grey_accessibilityTrait(UIAccessibilityTraitButton),
    ])!

    var error: NSError?
    G.select(elementWithMatcher: expandItemMatcher)
      .inRoot(grey_kindOfClass(UINavigationBar.self))
      .perform(grey_tap(), error: &error)
